  ...
  const AdminLogin = () => {
    ...
    }, [bookings, filterDate, filterDoctor, filterLocation, filterName]);
    ...
    const fetchBookings = async () => {
      ...
      try {
        // Simplified query to ensure we get all bookings
    ...
    // Fix for the applyBookingsFilters function
  const applyBookingsFilters = () => {
      let filtered = [...bookings];
      
      // Debug log to check initial bookings count
      ...
      // Apply doctor filter
      if (filterDoctor !== 'all') {
        filtered = filtered.filter(booking => booking.doctorType === filterDoctor);
      //   console.log("After doctor filter:", filtered.length);
      }
  ...
  const handleAcceptConsultation = async (booking) => {
    try {
      ...
      const templateParams = {
        ...
        doctor_phone: booking.doctorType === 'dr_manish' ? '+91 98765 43210' : '+91 98765 43211',
  ...
  const handleDeclineConsultation = async (booking) => {
    ...
    if (!isAuthenticated) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-green-50 to-orange-50 py-12 px-4 sm:px-6 lg:px-8">
    ...
    // Admin dashboard
    return (
      <div className="min-h-screen bg-gradient-to-r from-green-50 to-green-50 py-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="hospital">Hospital (10AM-5PM)</option>
                  <option value="evening_clinic">Evening Clinic (5PM-8PM)</option>
                </select>
              </div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
            ...
            {isLoading ? (
              ...
              <div className="relative">
                <div 
                  className="overflow-y-auto max-h-96 scrollbar-track-transparent scrollbar-thumb-green-500 scrollbar-thin" 
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredBookings.map((booking) => (
                        <tr key={booking.id} className="hover:bg-green-50 transition duration-150">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              booking.doctorType === 'dr_manish' 
                                ? 'bg-purple-100 text-purple-800' 
                                : 'bg-pink-100 text-pink-800'
                            }`}>
                              {booking.doctorType === 'dr_manish' ? 'Dr. Manish' : 'Dr. Gauri'}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              ...
                              {booking.consultationType === 'hospital' ? 'Hospital' : 'Evening Clinic'}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
